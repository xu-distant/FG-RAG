...
from articles_retrieve.src import util
...
def load_retriever(model_path, pooling="average", random_init=False):
    ...
    if os.path.exists(path):
        pretrained_dict = torch.load(path, map_location="gpu")
        opt = pretrained_dict["opt"]
        print(opt.retriever_model_id)
        ...
        tokenizer = util.load_hf(transformers.AutoTokenizer, retriever_model_id)
        cfg = util.load_hf(transformers.AutoConfig, retriever_model_id)
        ...
        if "xlm" in retriever_model_id:
            ...
            model_class = Contriever
        cfg = util.load_hf(transformers.AutoConfig, model_path)
        tokenizer = util.load_hf(transformers.AutoTokenizer, model_path)
        retriever = util.load_hf(model_class, model_path)
